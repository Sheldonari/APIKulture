...
void AppState::send_request() {
  ...
  pending_method_ = std::string(g.get_method());
  pending_url_ = std::string(g.get_url());
  pending_headers_ = std::string(g.get_request_headers());
  pending_body_ = std::string(g.get_request_body());
